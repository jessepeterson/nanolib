...
func (b *KVTxn) KeysPrefix(ctx context.Context, prefix string, cancel <-chan struct{}) <-chan string {
	return b.keysWithStagedKeys(b.store.KeysPrefix(ctx, prefix, cancel), cancel, prefix)
}

// stageKeys returns a slice of all staged keys.
// Optionally matches the prefix of the key.
...
func (b *KVTxn) keysWithStagedKeys(inKeys <-chan string, cancel <-chan struct{}, prefix string) <-chan string {
	r := make(chan string)
	go func() {
		defer close(r)
		for k := range inKeys {
